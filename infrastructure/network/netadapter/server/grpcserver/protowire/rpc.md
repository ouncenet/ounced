...
Clients are expected to build RequestMessages and wrap them in OuncedMessage. (see messages.proto)

Having received a RequestMessage, (wrapped in a OuncedMessage) the RPC server will respond with a
ResponseMessage (likewise wrapped in a OuncedMessage) respective to the original RequestMessage.
...
GetCurrentNetworkRequestMessage requests the network ounced is currently running against.
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| payAddress | [string](#string) |  | Which ounce address should the coinbase block reward transaction pay into |
...
| ----- | ---- | ----- | ----------- |
| block | [RpcBlock](#protowire.RpcBlock) |  |  |
| isSynced | [bool](#bool) |  | Whether ounced thinks that it&#39;s synced. Callers are discouraged (but not forbidden) from solving blocks when ounced is not synced. That is because when ounced isn&#39;t in sync with the rest of the network there&#39;s a high chance the block will never be accepted, thus the solving effort would have been wasted. |
| error | [RPCError](#protowire.RPCError) |  |  |
...
GetPeerAddressesRequestMessage requests the list of known ounced addresses in the
...
currently connected to this ounced.
...
| isOutbound | [bool](#bool) |  | Whether this ounced initiated the connection |
| timeOffset | [int64](#int64) |  |  |
| userAgent | [string](#string) |  |  |
| advertisedProtocolVersion | [uint32](#uint32) |  | The protocol version that this peer claims to support |
| timeConnected | [int64](#int64) |  | The timestamp of when this peer connected to this ounced |
...
AddPeerRequestMessage adds a peer to ounced&#39;s outgoing connection list.
This will, in most cases, result in ounced connecting to said peer.
...
parent chain from some startHash to this ounced&#39;s current virtual
...
GetBlockCountRequestMessage requests the current number of blocks in this ounced.
...
ShutDownRequestMessage shuts down this ounced.
...
This call is only available when this ounced was started with `--utxoindex`
...
GetUtxosByAddressesRequestMessage requests all current UTXOs for the given ounced addresses
