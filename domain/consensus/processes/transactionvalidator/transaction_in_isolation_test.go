...
func TestValidateTransactionInIsolationAndPopulateMass(t *testing.T) {
	testutils.ForAllNets(t, true, func(t *testing.T, consensusConfig *consensus.Config) {
		...
		tests := []struct {
			...
			{"too much grain in one output", 1, 1, constants.MaxGrain + 1,
				subnetworks.SubnetworkIDNative,
				nil,
				nil,
				ruleerrors.ErrBadTxOutValue, 0},
			{"too much grain before- valid now", 1, 1, 21e14 + 1,
				subnetworks.SubnetworkIDNative,
				nil,
				nil,
				nil, 0},
			{"too much grain in one output - after hf", 1, 1, constants.MaxGrain + 1,
				subnetworks.SubnetworkIDNative,
				nil,
				nil,
				ruleerrors.ErrBadTxOutValue, 0},
			...
			{"non-zero gas in subnetwork registry", 1, 1, 1,
				...
				nil,
				ruleerrors.ErrInvalidGas, 0},
			{"non-zero payload in Ounce", 1, 1, 1,
				subnetworks.SubnetworkIDNative,
				nil,
				func(tx *externalapi.DomainTransaction) {
					tx.Payload = []byte{1}
