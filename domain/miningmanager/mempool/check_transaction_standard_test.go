...
func TestIsTransactionOutputDust(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			externalapi.DomainTransactionOutput{Value: constants.MaxGrain, ScriptPublicKey: scriptPublicKey},
			util.Amount(1000),
			false,
		},
		{
...
func TestCheckTransactionStandardInIsolation(t *testing.T) {
	...
	addr, err := util.NewAddressPublicKey(addrHash[:], util.Bech32PrefixOunceTest)
	if err != nil {
