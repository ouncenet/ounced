...
func (mp *mempool) checkTransactionStandardInIsolation(transaction *externalapi.DomainTransaction) error {
	...
	// However, in a later version of Ounce the consensus-valid transaction version range might diverge from the
...
func (mp *mempool) IsTransactionOutputDust(output *externalapi.DomainTransactionOutput) bool {
	...
	// mp.config.MinimumRelayTransactionFee is in grain/KB, so multiply
...
func (mp *mempool) minimumRequiredTransactionRelayFee(mass uint64) uint64 {
	...
	// grain/kg so multiply by mass (which is in grams) and divide by 1000 to get minimum grains.
	...
	if minimumFee > constants.MaxGrain {
		minimumFee = constants.MaxGrain
	}
