/*
Package appmessage implements the ounce appmessage protocol.

At a high level, this package provides support for marshalling and unmarshalling
supported ounce messages to and from the appmessage. This package does not deal
with the specifics of message handling such as what to do when a message is
received. This provides the caller with a high level of flexibility.

# Kaspa Message Overview

The ounce protocol consists of exchanging messages between peers. Each
message is preceded by a header which identifies information about it such as
which ounce network it is a part of, its type, how big it is, and a checksum
...
of the currently supported ounce messages are provided. For these supported
messages, all of the details of marshalling and unmarshalling to and from the
appmessage using ounce encoding are handled so the caller doesn't have to concern
themselves with the specifics.

# Message Interaction

The following provides a quick summary of how the ounce messages are intended
...
and write ounce messages. The following sections provide a quick overview of
...
The ounce network is a magic number which is used to identify the start of a
message and which ounce network the message applies to. This package provides
...
As discussed in the ounce message overview section, this package reads
and writes ounce messages using a generic interface named Message. In
...
In order to unmarshall ounce messages from the appmessage, use the ReadMessage
function. It accepts any io.Reader, but typically this will be a net.Conn to
a remote node running a ounce peer. Example syntax is:
	...
	msg, rawPayload, err := appmessage.ReadMessage(conn, pver, ounceNet)
	if err != nil {
...
In order to marshall ounce messages to the appmessage, use the WriteMessage
function. It accepts any io.Writer, but typically this will be a net.Conn to
a remote node running a ounce peer. Example syntax to request addresses
from a remote peer is:
	...
	// error.
	err := appmessage.WriteMessage(conn, msg, pver, ounceNet)
	if err != nil {
		// Log and handle the error
	}
