...
// Outpoint defines a ounce data type that is used to track previous
// transaction outputs.
...
// NewOutpoint returns a new ounce transaction outpoint point with the
...
// TxIn defines a ounce transaction input.
type TxIn struct {
...
// NewTxIn returns a new ounce transaction input with the provided
...
// TxOut defines a ounce transaction output.
type TxOut struct {
	Value        uint64
	ScriptPubKey *externalapi.ScriptPublicKey
}

// NewTxOut returns a new ounce transaction output with the provided
...
// MsgTx implements the Message interface and represents a ounce tx message.
