...
func TestMultisig(t *testing.T) {
	testutils.ForAllNets(t, true, func(t *testing.T, consensusConfig *consensus.Config) {
		...
		forSchnorrAndECDSA(t, func(t *testing.T, ecdsa bool) {
			...
			path := "m/1/2/3"
			address, err := libouncewallet.Address(params, publicKeys, minimumSignatures, path, ecdsa)
			if err != nil {
			...
			isFullySigned, err := libouncewallet.IsTransactionFullySigned(unsignedTransaction)
			if err != nil {
			...
			_, err = libouncewallet.ExtractTransaction(unsignedTransaction, ecdsa)
			...
			signedTxStep1, err := libouncewallet.Sign(params, mnemonics[:1], unsignedTransaction, ecdsa)
			if err != nil {
				t.Fatalf("Sign: %+v", err)
			}

			isFullySigned, err = libouncewallet.IsTransactionFullySigned(signedTxStep1)
			if err != nil {
				t.Fatalf("IsTransactionFullySigned: %+v", err)
			}
			...
			signedTxStep2, err := libouncewallet.Sign(params, mnemonics[1:2], signedTxStep1, ecdsa)
			if err != nil {
				t.Fatalf("Sign: %+v", err)
			}

			extractedSignedTxStep2, err := libouncewallet.ExtractTransaction(signedTxStep2, ecdsa)
			if err != nil {
				t.Fatalf("ExtractTransaction: %+v", err)
			}

			signedTxOneStep, err := libouncewallet.Sign(params, mnemonics[:2], unsignedTransaction, ecdsa)
			if err != nil {
				t.Fatalf("Sign: %+v", err)
			}

			extractedSignedTxOneStep, err := libouncewallet.ExtractTransaction(signedTxOneStep, ecdsa)
			if err != nil {
				t.Fatalf("ExtractTransaction: %+v", err)
			}
	...
	testutils.ForAllNets(t, true, func(t *testing.T, consensusConfig *consensus.Config) {
		...
		forSchnorrAndECDSA(t, func(t *testing.T, ecdsa bool) {
			...
			for i := 0; i < numKeys; i++ {
				...
				publicKeys[i], err = libouncewallet.MasterPublicKeyFromMnemonic(&consensusConfig.Params, mnemonics[i], false)
				if err != nil {
					t.Fatalf("MasterPublicKeyFromMnemonic: %+v", err)
				}
			}
			...
			signedTx, err := libouncewallet.Sign(params, mnemonics, unsignedTransaction, ecdsa)
			if err != nil {
				t.Fatalf("Sign: %+v", err)
			}

			tx, err := libouncewallet.ExtractTransaction(signedTx, ecdsa)
			if err != nil {
				t.Fatalf("ExtractTransaction: %+v", err)
			}
...
func TestMaxGrain(t *testing.T) {
	testutils.ForAllNets(t, true, func(t *testing.T, consensusConfig *consensus.Config) {
		...
		for i := 0; i < numKeys; i++ {
			...
			publicKeys[i], err = libouncewallet.MasterPublicKeyFromMnemonic(&cfg.Params, mnemonics[i], false)
			if err != nil {
				t.Fatalf("MasterPublicKeyFromMnemonic: %+v", err)
			}
		}
		...
		selectedUTXOsForTxWithLargeInputAmount := []*libouncewallet.UTXO{
			{
				Outpoint: &externalapi.DomainOutpoint{
		...
		unsignedTxWithLargeInputAmount, err := libouncewallet.CreateUnsignedTransaction(publicKeys, minimumSignatures,
			[]*libouncewallet.Payment{{
				Address: address,
				Amount:  10,
		...
		signedTxWithLargeInputAmount, err := libouncewallet.Sign(params, mnemonics, unsignedTxWithLargeInputAmount, false)
		if err != nil {
			t.Fatalf("Sign: %+v", err)
		}

		txWithLargeInputAmount, err := libouncewallet.ExtractTransaction(signedTxWithLargeInputAmount, false)
		if err != nil {
			t.Fatalf("ExtractTransaction: %+v", err)
		}
		...
		selectedUTXOsForTxWithLargeInputAndOutputAmount := []*libouncewallet.UTXO{
			{
				Outpoint: &externalapi.DomainOutpoint{
		...
		signedTxWithLargeInputAndOutputAmount, err := libouncewallet.Sign(params, mnemonics, unsignedTxWithLargeInputAndOutputAmount, false)
		if err != nil {
			t.Fatalf("Sign: %+v", err)
		}

		txWithLargeInputAndOutputAmount, err := libouncewallet.ExtractTransaction(signedTxWithLargeInputAndOutputAmount, false)
		if err != nil {
			t.Fatalf("ExtractTransaction: %+v", err)
		}
