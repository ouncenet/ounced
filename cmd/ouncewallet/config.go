...
type createConfig struct {
	KeysFile          string `long:"keys-file" short:"f" description:"Keys file location (default: ~/.ouncewallet/keys.json (*nix), %USERPROFILE%\\AppData\\Local\\Ouncewallet\\key.json (Windows))"`
...
type sendConfig struct {
	...
	Password                 string   `long:"password" short:"p" description:"Wallet password"`
	DaemonAddress            string   `long:"daemonaddress" short:"d" description:"Wallet daemon server to connect to"`
	ToAddress                string   `long:"to-address" short:"t" description:"The public address to send Ounce to" required:"true"`
	FromAddresses            []string `long:"from-address" short:"a" description:"Specific public address to send Ounce from. Repeat multiple times (adding -a before each) to accept several addresses" required:"false"`
	SendAmount               string   `long:"send-amount" short:"v" description:"An amount to send in Ounce (e.g. 1234.12345678)"`
	IsSendAll                bool     `long:"send-all" description:"Send all the Ounce in the wallet (mutually exclusive with --send-amount). If --from-address was used, will send all only from the specified addresses."`
...
type createUnsignedTransactionConfig struct {
	DaemonAddress            string   `long:"daemonaddress" short:"d" description:"Wallet daemon server to connect to"`
	ToAddress                string   `long:"to-address" short:"t" description:"The public address to send Ounce to" required:"true"`
	FromAddresses            []string `long:"from-address" short:"a" description:"Specific public address to send Ounce from. Use multiple times to accept several addresses" required:"false"`
	SendAmount               string   `long:"send-amount" short:"v" description:"An amount to send in Ounce (e.g. 1234.12345678)"`
	IsSendAll                bool     `long:"send-all" description:"Send all the Ounce in the wallet (mutually exclusive with --send-amount)"`
	UseExistingChangeAddress bool     `long:"use-existing-change-address" short:"u" description:"Will use an existing change address (in case no change address was ever used, it will use a new one)"`
	config.NetworkFlags
}
...
func parseCommandLine() (subCommand string, config interface{}) {
	...
	parser.AddCommand(balanceSubCmd, "Shows the balance of a public address",
		"Shows the balance for a public address in Ounce", balanceConf)

	sendConf := &sendConfig{DaemonAddress: defaultListen}
	parser.AddCommand(sendSubCmd, "Sends a Ounce transaction to a public address",
		"Sends a Ounce transaction to a public address", sendConf)
	...
	parser.AddCommand(createUnsignedTransactionSubCmd, "Create an unsigned Ounce transaction",
		"Create an unsigned Ounce transaction", createUnsignedTransactionConf)
