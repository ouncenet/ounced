...
// Takes in a string representation of the Kas value to convert to Grain
func TestKasToGrain(t *testing.T) {
	...
	for _, currentTestVector := range validCases {
		convertedAmount, err := KasToGrain(currentTestVector.originalAmount)
	...
	for _, currentTestVector := range invalidCases {
		_, err := KasToGrain(currentTestVector)
...
func TestValidateAmountFormat(t *testing.T) {
	validCases := []string{
		...
		"184467440737.09551615", // Maximum input that can be represented in grain later
		"184467440737.09551616", // Cannot be represented in grain, but we'll acccept for "correct format"
		"999999999999.99999999", // Cannot be represented in grain, but we'll acccept for "correct format"
	}

	for _, testCase := range validCases {
		err := validateOZAmountFormat(testCase)
