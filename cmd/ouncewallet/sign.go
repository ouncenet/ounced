...
func sign(conf *signConfig) error {
	...
	for i, partiallySignedTransaction := range partiallySignedTransactions {
		updatedPartiallySignedTransactions[i], err =
			libouncewallet.Sign(conf.NetParams(), privateKeys, partiallySignedTransaction, keysFile.ECDSA)
		if err != nil {
			return err
		}
	}
	...
	for _, updatedPartiallySignedTransaction := range updatedPartiallySignedTransactions {
		...
		isFullySigned, err := libouncewallet.IsTransactionFullySigned(updatedPartiallySignedTransaction)
		if err != nil {
			return err
		}
		if !isFullySigned {
