...
// 2. A wrapper that implements isOuncedMessage_Payload, having a single field pointing to the concrete command
//    bearing the name of the command with `OuncedMessage_` prefix and `Request` suffix
...
// generateOuncedMessage generates a wrapped OuncedMessage with the given `commandValue`
func generateOuncedMessage(commandValue reflect.Value, commandDesc *commandDescription) (*protowire.OuncedMessage, error) {
	commandWrapper := reflect.New(commandDesc.typeof)
	unwrapCommandValue(commandWrapper).Set(commandValue)

	kaspadMessage := reflect.New(reflect.TypeOf(protowire.OuncedMessage{}))
	kaspadMessage.Elem().FieldByName("Payload").Set(commandWrapper)
	return kaspadMessage.Interface().(*protowire.OuncedMessage), nil
}
